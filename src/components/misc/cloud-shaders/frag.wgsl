...
const scrollSpeed = vec2<f32>(0.036, 0.0);
...
fn main(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    let brightness = uvColor.r * 1.6 + uv.x * 5.4 - 0.36;

    let rgb = select(mix(0.118, 0.0392, (brightness - 0.98) * 50.0), 0.0196, brightness < 0.98);
