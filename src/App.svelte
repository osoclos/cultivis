<script lang="ts">
    ...
    import { bishopData, hereticData, knucklebonesPlayerData, machineData, miniBossData, questGiverData, shopkeeperData, towwData } from "./data/files";
    import { BISHOP_IDS, GUARD_IDS, HERETIC_IDS, KNUCKLEBONES_PLAYER_IDS, MACHINE_IDS, MINI_BOSS_IDS, OCCULTIST_IDS, QUEST_GIVER_IDS, SHOPKEEPER_IDS, SOLDIER_IDS, WITNESS_IDS } from "./data/types";
    ...
    async function addActor(actor: typeof Actor, updateActorIdx: boolean = true) {
        ...
        switch (actor) {
            ...
            case Shopkeeper: {
                const id = Random.item(SHOPKEEPER_IDS);
                !factory.hasLoadedShopkeeper(id) && await factory.loadShopkeeper(id) && await exporter.factory.loadShopkeeper(id);

                const shopkeeper = factory.shopkeeper(id);
                shopkeeper.setRawAnimation(shopkeeperData[id].animation);

                addedActor = shopkeeper;
                break;
            }
    ...
    {#await newsManager.areTermsAcknowledged() then areTermsAcknowledged}
        {#if !areTermsAcknowledged}
            {#await Promise.all([newsManager.getTermsSummary(), newsManager.getTermsUnix()]) then [changesSummary, termsUnix]}
                <div class="{hasFinishedLoading ? "hidden" : "grid"} fixed top-0 left-0 z-100 place-items-center w-full h-full bg-[#00000060] {hasUserCompliedToTOS ? "opacity-0" : "opacity-100"} transition-opacity duration-450 select-none" ontransitionend={({ target }) => (target as HTMLElement).classList.replace("grid", "hidden")}>
            ...
            {#if categoryIdx === 0}
                ...
                <div class={["lg:absolute z-90 lg:top-0 w-full lg:w-160 lg:h-full bg-secondary transition-[left,_filter] motion-reduce:transition-opacity duration-500", !showActorMenu ? "lg:-left-210 lg:motion-reduce:left-0 lg:brightness-0 lg:motion-reduce:brightness-100 lg:motion-reduce:opacity-0 lg:ease-in lg:motion-reduce:pointer-events-none" : "lg:left-0 lg:brightness-100 lg:motion-reduce:opacity-100 lg:ease-out", { "not-lg:hidden": !showActorMenu }]}>
                    {#if actor && actorObj && actorMenu}
                        ...
                        {:else if isShopkeeperObj(actorObj) && actorMenu === SHOPKEEPER_MENU_NAME}
                            <ShopkeeperMenus class="no-scrollbar lg:overflow-y-auto lg:pt-12 lg:pb-8 lg:w-160 lg:h-[calc(100%_-_68px)]" obj={actorObj} {factory} enableKeyInput={actorIdx >= 0 && showActorMenu && !isRiverBoyObituaryVisible} onupdate={updateSceneFromChanges} onchange={swapActor} />
                        {/if}
                    {/if}
                </div>
